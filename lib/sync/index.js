...
function connect(mongoDBConnectionUrl, redisUrl, cb) {
  if (arguments.length < 3) throw new Error('connect requires 3 arguments');
...
function start(cb) {
  ...
  async.waterfall([
    ...
    function createWorkers(callback) {
      ackWorker = new Worker(ackQueue, ackProcessor);
      pendingWorker = new Worker(pendingQueue, pendingProcessor);
      syncWorker = new Worker(syncQueue, syncProcessor);
      return callback();
    },
