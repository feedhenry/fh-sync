...
function syncWithBackend(payload, callback) {
  ...
  async.waterfall([
    ...
    function listData(cb) {
      listDataWithTimeout({datasetClientId: datasetClientId, datasetId: datasetId, queryParams: queryParams, metaData: metaData}, expectedTimeout, cb);
    },
    ...
    syncStorage.updateDatasetClient(datasetClientId, {syncLoopEnd: Date.now()}, function(err){
      if (err) {
