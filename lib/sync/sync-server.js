...
function start(cb) {
  ...
  async.series([
    ...
    function createWorkers(callback) {
      ...
      syncWorker = new Worker(syncQueue, syncProcessorImpl, metricsClient, {name: 'sync_worker', interval: syncConfig.syncWorkerInterval, collectStatsInterval: syncConfig.collectStatsInterval});

      var ackProcessorImpl = ackProcessor(syncStorage);
      ackWorker = new Worker(ackQueue, ackProcessorImpl, metricsClient, {name: 'ack_worker', interval: syncConfig.ackWorkerInterval, collectStatsInterval: syncConfig.collectStatsInterval});

      var pendingProcessorImpl = pendingProcessor(syncStorage, dataHandlers, hashProvider, metricsClient);
      pendingWorker = new Worker(pendingQueue, pendingProcessorImpl, metricsClient, {name: 'pending_worker', interval: syncConfig.pendingWorkerInterval, collectStatsInterval: syncConfig.collectStatsInterval});
