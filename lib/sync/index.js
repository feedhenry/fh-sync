...
function connect(mongoDBConnectionUrl, redisUrl, metricsConf, cb) {
  ...
  async.series([
    ...
    metrics = metricsModule.init(metricsConf);
    return cb(null, mongoDbClient, redisClient, metrics);
  });
}
...
function start(cb) {
  ...
  async.waterfall([
    function createQueues(callback) {
      ackQueue = new MongodbQueue('sync-ack-queue', metrics, {mongodb: mongoDbClient});
      pendingQueue = new MongodbQueue('sync-pending-queue', metrics, {mongodb: mongoDbClient});
      syncQueue = new MongodbQueue('sync-queue', metrics, {mongodb: mongoDbClient});
    ...
    function createWorkers(callback) {
      ackWorker = new Worker(ackQueue, ackProcessor, metrics, {name: 'ack_worker'});
      pendingWorker = new Worker(pendingQueue, pendingProcessor, metrics, {name: 'pending_worker'});
      syncWorker = new Worker(syncQueue, syncProcessor, metrics, {name: 'sync_worker'});
      ackWorker.work();
      pendingWorker.work();
      syncWorker.work();
      return callback();
