...
MongodbQueue.prototype.prune = function(cb) {
  ...
  collection.deleteMany(pruneQuery, function(err, result){
    if (err) {
      syncUtil.doLog(syncUtil.SYNC_LOGGER, 'error', 'Failed to delete messages from queue ' + self.queueName + ' due to error ' + util.inspect(err));
    } else {
      syncUtil.doLog(syncUtil.SYNC_LOGGER, 'debug', 'Deleted ' + result.deletedCount + ' messages from queue ' + self.queueName);
    }
