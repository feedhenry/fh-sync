...
var DEFAULT_SYNC_CONF = {
  ...
  /** @type {Object} the backoff strategy for the pending worker to use. Default is exponetial, and the max delay is 60s */
  ...
  /** @type {Object} the backoff strategy for the ack worker to use. Default is exponetial, and the max delay is 60s */
  ...
  /** @type {Object} the backoff strategy for the sync worker to use. Default is exponetial, and the max delay is 60s */
  syncWorkerBackoff: {strategy: 'exp', max: 60*1000},
...
function start(cb) {
  ...
  async.series([
    ...
    function createWorkers(callback) {
      ...
      syncWorker = new Worker(syncQueue, syncProcessorImpl, metricsClient, syncWorkerOpts);
      ...
      ackWorker = new Worker(ackQueue, ackProcessorImpl, metricsClient, ackWorkerOpts);
      ...
      pendingWorker = new Worker(pendingQueue, pendingProcessorImpl, metricsClient, pendingWorkerOpts);
