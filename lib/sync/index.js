...
var SyncScheduler = require('./sync-scheduler').SyncScheduler;
var datasetClientsCacheKey = 'sync:datasetClients';
...
function start(cb) {
  ...
  syncStorage = storageModule(redisClient);
  ...
  async.waterfall([
    ...
    function startSyncScheduler(callback) {
      syncScheduler = new SyncScheduler(redisClient, datasetClientsCacheKey);
      return callback();
    }
  ], function(err) {
    if (err) return cb(err);
