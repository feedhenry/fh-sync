  ...
  async.waterfall([
    ...
    function filterDatasetClients(datasetClients, callback) {
      var datasetClientsToRemove = _.filter(datasetClients, function(datasetClientJson){
        ...
        return datasetClient.shouldBeRemoved(parseDuration(retentionPeriod));
      });
    ...
    if (err) {
      syncUtil.doLog(syncUtil.SYNC_LOGGER, 'warn', 'Failed to cleanup dataset clients due to error ' + util.inspect(err));
    } else {
      syncUtil.doLog(syncUtil.SYNC_LOGGER, 'debug', 'Removed ' + _.size(deletedDatasetClients) + ' dataset clients');
    }
  });
}
