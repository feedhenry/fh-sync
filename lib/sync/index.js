...
function start(mongoClient, cb) {
  // use the last arg as a cb. There should *always* be a function as the last param,
  ...
  // In that case, call the callback with an error


  // if anything goes wrong from here on, call the callback with an error.
  // we'll need to wait for the mongo client to have a connection before continuing though.
  // (unless a mongo client was passed in, in which case we'll assume its already connected)
