...
SyncScheduler.prototype.checkDatasetsForSyncing = function(cb) {
  ...
  syncStorage.listDatasetClients(function(err, datasetClients){
    ...
    if (err) {
      timing = timer.stop();
      metricsClient.gauge(metrics.KEYS.SYNC_SCHEDULER_CHECK_TIME, {success: false}, timing);
      return cb(err);
    }
    ...
    _.each(datasetClients, function(datasetClientJson){
      ...
      var dataset_id = datasetClient.getDatasetId();
      if (datasetClient.shouldSync()) {
        syncUtil.doLog(dataset_id, 'verbose', 'schedule sync run');
        ...
        syncRequest.startTime = new Date().getTime();
        datasetClientsToSync.push(syncRequest);
      } else if (datasetClient.shouldDeactiveSync()) {
        syncUtil.doLog(dataset_id, 'info', 'Deactivating sync for client ' + datasetClient.getDatasetId() + '. No client instances have accessed in ' + datasetClient.getConfig().clientSyncTimeout * 1000 + 'ms');
    ...
    async.series([function addDatasetClientsToSyncQueue(wcb) {
      ...
      timing = timer.stop();
      metricsClient.gauge(metrics.KEYS.SYNC_SCHEDULER_CHECK_TIME, {success: true}, timing);
      return cb();
    });
  });
};
