...
function listDatasetClientRecords(datasetClientJson, projection, cb) {
  ...
  if (recordUids.length > 0) {
    ...
    return cursor.toArray(function(err, records) {
      if (err) {
        syncUtil.doLog(syncUtil.SYNC_LOGGER, 'error', 'Failed to list datasetClient records due to error ' + util.inspect(err) + ' :: datasetClientJson = ' + util.inspect(datasetClientJson));
      }
...
function doUpdateManyDatasetClients(query, fields, callback) {
  ...
  datasetClientsCollection.updateMany(query, {$set: fields}, function(err, result) {
    if (err) {
      syncUtil.doLog(query, 'error', ' Failed to doUpdateManyDatasetClients due to error ' + util.inspect(err) + ' :: fields = ' + util.inspect(fields));
      return callback(err);
    }
